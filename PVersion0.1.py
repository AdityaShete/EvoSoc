...
class human:
    class strenght:
        ...
        def increasestrenght(self):#human should increase his strenght only below a threshold
            while self.currStrenght <= C.strenghtThreshold and world.Resource[human.posx][human.posy]>0:
                human.hstrength += 1
                world.Resource[human.posx][human.posy] -= 1
    ...
    def _init_(self):#Human intialisation contains parameters that have to be varied
        ...
        self.posx = 1
        self.posy = 1
        self.positionmatrix.append([posx,posy])
        
    def initchild(self,father,mother):
        ...
        self.posx = father.posx
        self.posy = father.posy
        self.positionmatrix.append([posx,posy])
    
       
    def move(self):
        #how to move a human so that he doesnt come back to his original positions?
        if self.curiosity>self.curiosityThresh:
            while [self.posx,self.posy] in self.positionmatrix:
                self.posx = self.posx + rd.randint(-1,1)
                self.posy = self.posy + rd.randint(-1,1)
            self.positionmatrix.append([posx,posy]) 
        elif world.Resource[self.posx][self.posy] == 0:
            while 
